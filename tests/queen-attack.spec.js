import { QueenAttack } from '../src/queen-attack.js';
import {expect} from 'chai';

describe('Queens', () => {
  describe('it creation of Queens with valid and invalid positions', () => {
    it('queen with a valid position', () => {
      const queens = new QueenAttack({ white: [2, 2] });
      expect(queens.white).to.be.eql([2, 2]);
    });

    it('queen with a valid position', () => {
      const queens = new QueenAttack({ black: [2, 2] });
      expect(queens.black).to.be.eql([2, 2]);
    });

    it('queen must have positive row', () => {
      const positioning = { white: [-2, 2] };
      const expectedError = 'Queen must be placed on the board';
      expect(() => new QueenAttack(positioning)).to.throw(expectedError);
    });
  ...
  describe('it the board visualisation', () => {
    it('board', () => {
      ...
      expect(queens.toString()).to.be.eql(board);
    });
